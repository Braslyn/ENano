...
declare(dec(method,Type,Id,Body),R ):-  mbody(Body,R1),dtype2(Type,Ty),format(atom(R),'~s ~s ~s',[Ty,Id,R1]).
...
mbody(lambda(Id,body(dIf([F],P,B))),R):- format(atom(R),'= ~s -> ( ~q )? ~q : ~q',[Id,P,F,B]).
dtype2(type(arrow(_,Y)),R):- format(atom(R),'~s',[Y]).
